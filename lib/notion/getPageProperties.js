...
export default async function getPageProperties(id, block, schema, authToken, tagOptions) {
  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    const propVal = Array.isArray(val) ? val : []
    properties.id = id
    if (safeSchema[key]?.type && !excludeProperties.includes(safeSchema[key].type)) {
      properties[safeSchema[key].name] = getTextContent(propVal)
    } else {
      switch (safeSchema[key]?.type) {
        case 'date': {
          const dateProperty = getDateValue(propVal)
          delete dateProperty.type
          properties[safeSchema[key].name] = dateProperty
          break
        }
        case 'select':
        case 'multi_select': {
          const selects = getTextContent(propVal)
          if (selects[0]?.length) {
            properties[safeSchema[key].name] = selects.split(',')
          }
          break
        }
        case 'person': {
          const rawUsers = propVal.flat()
          const users = []
          ...
          properties[safeSchema[key].name] = users
          break
        }
        default:
          break
