...
export async function getGlobalNotionData({
  ...
  // 获取Notion数据
  const rawData = await getNotionPageData({ pageId, from })
  if (!rawData) {
    console.warn('[NotionData]', `获取数据为空, from:${from}, pageId:${pageId}，请检查NOTION_PAGE_ID是否为数据库页面、token是否有效`)
    return {
      notice: null,
      siteInfo: { title: BLOG.TITLE, description: BLOG.DESCRIPTION, pageCover: BLOG.HOME_BANNER_IMAGE, icon: BLOG.AVATAR },
      allPages: [],
      collection: null,
  ...
  const notionPageData = deepClone(rawData)
...
async function getPageRecordMapByNotionAPI({ pageId, from }) {
  ...
  if (
    ...
    console.error(`pageId "${pageId}" is not a database (type: ${rawMetadata?.type || 'unknown'})。请确认:\n1. NOTION_PAGE_ID 指向的是一个数据库(Database)页面，而非普通Page\n2. 如果是私有页面，请检查 NOTION_TOKEN_V2 是否有效未过期\n3. Notion内部API可能有变动，请尝试重新获取页面ID`)
    return null
  }
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    try {
      ...
      console.error(`[Notion row parse failed] pageId=${id}`, error)
    }
  }
