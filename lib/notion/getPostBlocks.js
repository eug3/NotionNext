...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      return normalizeNotionRecordMap(pageData)
    } catch (e) {
      console.warn('[响应异常]:', e)
      // 增加延迟时间，根据剩余重试次数递增延迟
...
/**
 * 规范化 Notion API 返回的 RecordMap 数据结构
 * Notion 内部 API 在 2025 年变更了返回格式，block 和 collection 数据多了一层 value 嵌套:
 * 旧格式: block[id] = { value: { id, type, ... }, role: 'editor' }
 * 新格式: block[id] = { spaceId: '...', value: { value: { id, type, ... } } }
 * 此函数将新格式转换为旧格式，确保下游代码兼容
 */
function normalizeNotionRecordMap(recordMap) {
  ...
  if (recordMap.block) {
    for (const key of Object.keys(recordMap.block)) {
      ...
      // 检测新格式：value 里嵌套了 value，且外层 value 没有 type（旧格式 value 直接有 type）
      if (entry?.value?.value && typeof entry.value.value === 'object' && !entry.value.type) {
        recordMap.block[key] = {
          value: entry.value.value,
          role: entry.role || entry.value.role || 'reader'
        }
      }
    }
  }
  ...
  if (recordMap.collection) {
    for (const key of Object.keys(recordMap.collection)) {
      const entry = recordMap.collection[key]
      // 检测新格式：collection value 嵌套了 value，且外层 value 没有 schema（旧格式 value 直接有 schema）
      if (entry?.value?.value && typeof entry.value.value === 'object' && !entry.value.schema) {
        recordMap.collection[key] = {
          value: entry.value.value,
          role: entry.role || entry.value.role || 'reader'
        }
      }
